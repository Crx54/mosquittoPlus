...
static int net__try_connect_tcp(const char *host, uint16_t port, mosq_sock_t *sock, const char *bind_address,
                                bool blocking) {
	struct addrinfo hints;
	struct addrinfo *ainfo, *rp;
...
#  endif
	...
	if (mosq->tls_cafile) {
		...
		if (ret == 0) {
#  ifdef WITH_BROKER
            log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load CA certificates, check bridge_cafile \"%s\".",
                        mosq->tls_cafile);
#  else
			log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load CA certificates, check cafile \"%s\".",
			            mosq->tls_cafile);
#  endif
			return MOSQ_ERR_TLS;
		}
	...
	if (mosq->tls_capath) {
		...
		if (ret == 0) {
#  ifdef WITH_BROKER
            log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load CA certificates, check bridge_capath \"%s\".",
                        mosq->tls_capath);
#  else
			log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load CA certificates, check capath \"%s\".",
			            mosq->tls_capath);
#  endif
			return MOSQ_ERR_TLS;
		}
	...
	if (mosq->user_ssl_ctx) {
		...
		if (!mosq->ssl_ctx_defaults) {
			...
			log__printf(mosq, MOSQ_LOG_ERR,
			            "Error: If you use MOSQ_OPT_SSL_CTX then MOSQ_OPT_SSL_CTX_WITH_DEFAULTS must be true, or at least one of cafile, capath or psk must be specified.");
			return MOSQ_ERR_INVAL;
		}
	}
#endif
...
#  endif
			...
			if (mosq->tls_certfile) {
				...
				if (ret != 1) {
                    ...
                    log__printf(mosq, MOSQ_LOG_ERR,
                                ...
                                mosq->tls_certfile);
#else
					log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load client certificate \"%s\".",
					            mosq->tls_certfile);
#endif
#if !defined(OPENSSL_NO_ENGINE)
					ENGINE_FINISH(engine);
			...
			if (mosq->tls_keyfile) {
				if (mosq->tls_keyform == mosq_k_engine) {
					...
					if (!pkey) {
						log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load engine private key file \"%s\".",
						            mosq->tls_keyfile);
						ENGINE_FINISH(engine);
						net__print_ssl_error(mosq);
						return MOSQ_ERR_TLS;
					...
					if (SSL_CTX_use_PrivateKey(mosq->ssl_ctx, pkey) <= 0) {
						log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to use engine private key file \"%s\".",
						            mosq->tls_keyfile);
						ENGINE_FINISH(engine);
						net__print_ssl_error(mosq);
						return MOSQ_ERR_TLS;
					...
					if (ret != 1) {
                        ...
                        log__printf(mosq, MOSQ_LOG_ERR,
                                    ...
                                    mosq->tls_keyfile);
#else
						log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load client key file \"%s\".",
						            mosq->tls_keyfile);
#endif
#if !defined(OPENSSL_NO_ENGINE)
						ENGINE_FINISH(engine);
