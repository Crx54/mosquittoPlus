...
static void read_callback_client(EV_P_ ev_io *w, int revents) {
...
static void timeout_callback_client(EV_P_ ev_timer *w, int revents) {
    struct simple_client *client = w->data;
...
int test_quic_client(int argc, char *argv[]) {
    if (argc < 3) {
    ...
    ev_init(&client.timer, timeout_callback_client);
    ...
    // Start event loop.
    ev_io watcher;
    ev_io_init(&watcher, read_callback_client, client.sock, EV_READ);
