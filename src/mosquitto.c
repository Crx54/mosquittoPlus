...
static int listeners__start_single_mqtt(struct mosquitto__listener *listener) {
	...
	listensock_new =
			mosquitto__realloc(listensock, sizeof(struct mosquitto__listener_sock) * (size_t) listensock_count);
	if (!listensock_new) {
		return 1;
	}
...
int main(int argc, char *argv[]) {
	...
	if (_setmaxstdio(8192) != 8192) {
		...
		if (_setmaxstdio(2048) != 2048) {
			log__printf(NULL, MOSQ_LOG_WARNING,
			            "Warning: Unable to increase maximum allowed connections. This session may be limited to 512 connections.");
		}
	}
#endif
