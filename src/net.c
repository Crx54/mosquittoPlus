...
struct mosquitto *net__socket_accept(struct mosquitto__listener_sock *listensock) {
	...
	if (new_context->listener->max_connections > 0 && new_context->listener->client_count > new_context->listener->
	    max_connections) {
		if (db.config->connection_messages == true) {
...
int net__tls_server_ctx(struct mosquitto__listener *listener) {
	...
	if (listener->tls_version == NULL) {
		...
		SSL_CTX_set_options(listener->ssl_ctx,
	...
	if (listener->ciphers_tls13) {
		...
		if (rc == 0) {
			log__printf(NULL, MOSQ_LOG_ERR,
			            "Error: Unable to set TLS 1.3 ciphersuites. Check cipher_tls13 list \"%s\".",
...
static int net__load_crl_file(struct mosquitto__listener *listener) {
	...
	if (rc < 1) {
		log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load certificate revocation file \"%s\". Check crlfile.",
		            listener->crlfile);
		net__print_error(MOSQ_LOG_ERR, "Error: %s");
		net__print_ssl_error(NULL);
		return MOSQ_ERR_TLS;
